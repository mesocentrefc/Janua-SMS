...
from janua.ws.api.db import get_contact_group_ids, get_group_ids, get_contact_ids

from janua.db.database import ContactGroup as ContactGroupTable

class ContactGroup(BaseApi):
    """
    Contact group REST API
    
    Expose database object: :class:`ContactGroup <janua.db.database.ContactGroup>`
    """
    methods = ['GET', 'POST', 'PUT', 'DELETE']
    authorized_roles = ['admin', 'supervisor']
    model = ContactGroupTable
    ...
    def get_single_preprocessor(admin, instance_id=None, **kw):
        ...
        """
        if instance_id and int(instance_id) not in get_contact_group_ids(admin):
    ...
    def get_many_preprocessor(admin, search_params=None, **kw):
        ...
        """
        contact_group_ids = get_contact_group_ids(admin)
        if len(contact_group_ids) == 0:
            contact_group_ids.append(0)

        if 'filters' in search_params:
            search_params['filters'].append({
                u'name': u'id', u'op': u'in', u'val': contact_group_ids
            })
        else:
    ...
    def post_preprocessor(admin, data=None, **kw):
        ...
        """
        if 'group_id' not in data or 'contact_id' not in data:
            raise ProcessingException(description='', code=400)

        if data['group_id'] not in get_group_ids(admin) or \
           data['contact_id'] not in get_contact_ids(admin):
